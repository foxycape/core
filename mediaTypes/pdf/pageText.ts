...
export const isPdfTextItem = (item: unknown): item is TextItem => {
    return typeof item === 'object' && item !== null && typeof (item as TextItem).str === 'string';
};

export const findAdjacentPdfTextItem = (items: unknown[], fromIndex: number, direction: -1 | 1): TextItem | null => {
...
export const isSamePdfTextLine = (prev: TextItem, next: TextItem): boolean => {
...
export const estimatePdfCharWidth = (item: TextItem): number => {
    return item.width / Math.max(item.str.length, 1);
};

export const computePdfHorizontalGap = (prev: TextItem, next: TextItem, spaceItem?: TextItem): number => {
 ...
 */
export const shouldKeepPdfCjSpace = (prev: TextItem, next: TextItem, spaceItem?: TextItem): boolean => {
...
export const trimSpuriousCjkTrailingSpace = (pageText: string, prev: TextItem, next: TextItem): string => {
...
export const appendPdfTextItem = (pageText: string, str: string, hasEOL: boolean, allowCRLF: boolean): string => {
...
export const appendPdfLineBreakIfNeeded = (pageText: string, item: TextItem, allowCRLF: boolean): string => {
...
export const buildPageTextWithCjWhitespaceHandling = (items: unknown[], allowCRLF: boolean): string => {
    let pageText = '';
...
export const mergeAxisAlignedRects = (a: PdfAxisAlignedRect, b: PdfAxisAlignedRect): PdfAxisAlignedRect => {
    return {
...
export const rectArea = (rect: PdfAxisAlignedRect): number => {
    return Math.max(0, rect.x2 - rect.x1) * Math.max(0, rect.y2 - rect.y1);
};

export const intersectRect = (a: PdfAxisAlignedRect, b: PdfAxisAlignedRect): PdfAxisAlignedRect | null => {
...
export const overlapRatioOnSmaller = (a: PdfAxisAlignedRect, b: PdfAxisAlignedRect): number => {
...
export const normalizePdfTextForDedupe = (str: string): string => {
    return str.replace(/\s+/g, '');
};

/** Texts are equal, or one contains the other (overlay split/merge differences) */
export const isCompatibleDuplicateText = (a: string, b: string): boolean => {
...
export const shouldSkipDedupeCandidate = (item: TextItem): boolean => {
 ...
 */
export const computeTextItemBoundsInUserSpace = (textItem: TextItem): PdfAxisAlignedRect => {
 ...
 */
export const dedupeOverlappingTextItemsWithBounds = <T extends { item: TextItem; bounds: PdfAxisAlignedRect }>(
    entries: T[]
): T[] => {
    const kept: T[] = [];
...
export const dedupePdfTextContentItems = (items: unknown[]): unknown[] => {
...
export const buildLineTextFromItems = (items: TextItem[], textFormat?: TextFormatOptions): string => {
...
export const computeTextItemBoundsInRenderSpace = (
    textItem: TextItem,

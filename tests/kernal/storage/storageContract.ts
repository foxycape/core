...
import type { IStorage } from '@/kernal/storage/IStorage'

export type StorageFactory = () => IStorage

export const describeStorageContract = (label: string, createStorage: StorageFactory) => {
  describe(`${label} IStorage contract`, () => {
    const opened: IStorage[] = []
    ...
    afterEach(async () => {
      ...
      await Promise.all(instances.map(async (instance) => {
        try {
          ...
          // ignore cleanup failures from already-disposed instances
        }
        try {
    ...
    it('lists created table names without exposing the internal array', async () => {
      const db = storage()
      await db.set('books', 'a', 1)
      ...
      names.splice(0)
      const again = await db.getTableNames()
      expect(again.map((name) => name.toLowerCase()).sort()).toEqual(['books', 'marks'])
    })
    ...
    it('drops the database so later reads miss', async () => {
      const db = storage()
      await db.set('books', 'a', 1)
      await db.dropDb()
      await expect(db.get('books', 'a')).resolves.toBeNull()
